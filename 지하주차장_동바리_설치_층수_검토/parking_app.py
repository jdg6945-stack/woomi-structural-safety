...
try:
    st.set_page_config(layout="wide", page_title="지하주차장 동바리 설치 층수 검토")
except Exception:
    pass
...
st.markdown("""
    ...
    .load-calc-total {
        font-size: 22px !important; font-weight: 800; color: #dc2626;
        margin-top: 15px; border-top: 1px dashed #e2e8f0; padding-top: 10px;
    }
    .beam-detail-item {
        font-size: 14px; color: #64748b; margin-left: 25px;
        border-left: 2px solid #3b82f6; padding-left: 12px; margin-bottom: 8px;
    }
    .highlight-thick {
        background-color: #fef3c7; padding: 2px 5px; border-radius: 4px;
...
if st.sidebar.button("단위모듈 평면도 보기 / 닫기", use_container_width=True):
...
if st.session_state.show_plan:
    ...
    if os.path.exists(_plan_path):
        try:
            img = Image.open(_plan_path).convert("RGB")
            ...
            st.sidebar.image(_plan_path, use_container_width=True)
...
if 'parking_df' not in st.session_state:
    st.session_state.parking_df = pd.DataFrame({
        ...
        "예상강도(MPa)": [30.0, 30.0, 30.0],
        "타설간격(일)": [28, 28, 28],
        COL_LOAD_DEAD: [0.3, 0.3, 0.3],
        COL_LOAD_LIVE: [6.0, 6.0, 6.0],
...
if len(df) != num_floors:
    if len(df) < num_floors:
        ...
        new_rows = pd.DataFrame({
            ...
            "타설간격(일)": [28]*diff, COL_LOAD_DEAD: [0.3]*diff, COL_LOAD_LIVE: [6.0]*diff, COL_LOAD_CONST: [1.0]*diff
        })
...
def update_parking_df_callback():
    if "parking_editor" in st.session_state:
        edited = st.session_state["parking_editor"]
        for row_idx, changes in edited["edited_rows"].items():
            for key, val in changes.items():
                # 층이름에서 '(타설층)' 접미사 제거
                if key == "층이름" and isinstance(val, str):
                    val = val.replace(" (타설층)", "").replace("(타설층)", "")
                st.session_state.parking_df.iat[row_idx, st.session_state.parking_df.columns.get_loc(key)] = val

st.markdown("<h1 style='color: #1e293b; font-size: 1.6rem; font-weight: 800; margin-bottom: 10px;'>지하주차장 동바리 설치 층수 검토</h1>", unsafe_allow_html=True)
...
with left_col:
    ...
    if len(df_for_edit) > 0:
        raw_val = str(df_for_edit.iloc[0, 0])
        if "(타설층)" not in raw_val:
            df_for_edit.iloc[0, 0] = f"{raw_val} (타설층)"
    st.data_editor(
        df_for_edit, 
        use_container_width=True, hide_index=True, num_rows="fixed",
        key="parking_editor", on_change=update_parking_df_callback
    )
    ...
    with st.expander("보 데이터 입력 (타설층)", expanded=False):
        st.markdown("""
            <div class="guide-box">
        ...
        with _bcol1:
            num_beams = st.number_input(f"보 수량", value=1, key="parking_num_beams")
        ...
        for i in range(num_beams):
            with tabs[i]:
                ...
                with c4:
                    bn = st.number_input(f"보 중복 개수", value=1, key=f"p_bn_{i}")
    ...
    def run_calculation(df_in, equiv_mm_sum, beam_logs):
        ...
        for i in range(1, len(df_in)):
            ...
            ld = float(row[COL_LOAD_DEAD]) if str(row[COL_LOAD_DEAD]) != "-" else 0.0
            l = float(row[COL_LOAD_LIVE]) if str(row[COL_LOAD_LIVE]) != "-" else 0.0
